...
class Data_Factory():
    ...
    def get_length(self):  # 得到user和item的最大长度
        length_user={}
        length_item={}
        f = open('../len80/dataset_len80_out.txt',encoding='utf-8')
        line = f.readline()
        while line:
            ...
            line = f.readline()
        f.close()
        return len(length_user),len(length_item)
    ...
    def construct_clc2school(self):
        ...
        for key in school_set:
            school_list[int(key)]=int(school_set[key])
        return school_list
    ...
    def split_user(self, ratio, path):
        ...
        trainpath = path+'/train_user.txt'
        testpath = path + '/test_user.txt'
        rating_map = defaultdict(set)
        datafile=open(path+'/dataset_len80_out.txt',encoding='utf-8')
        ...
        for key in rating_map.keys():
            ...
            temp_data=temp_data[np.lexsort(temp_data.T)] #sort by time
            if int(len(values) * ratio)>0:
                train = temp_data[:int(len(values) * ratio)]
                test = temp_data[int(len(values) * ratio):]
            else:
                train = temp_data[:1]
                test = []
    ...
    def construct_history(self,path):
        ...
        for i in range(self.item_field_M - self.item_bind_M):
            clc_index[i]=self.item_bind_M+i
        maxL=0
        ...
        for key in history_list:
            ...
            for value in values:
                ...
                total+=1
                maxL = maxL if num[int(value)-self.item_bind_M]<maxL else num[int(value)-self.item_bind_M]
            for i in range(len(num)):
                value_str+=str(i)+':'+str(num[i]*1.0/total)+' '
